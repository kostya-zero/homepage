...
function NavBarLink({ href, route, title, pathname }: NavBarLinkProps) {
    return (
        <Link
            href={href}
            className={`hover:text-custom-white font-medium transition duration-200 ${pathname === route ? "text-custom-white" : ""}`}
        >
...
export default function NavBar() {
    ...
    return (
        <header className="flex items-center justify-between mt-10 m-4">
            <Link href="/" className="text-xl flex flex-row text-white font-funnel font-black">
                <span className="text-neutral-400">K.</span>
                <span>ZERO</span>
            </Link>

            <div className="text-foreground-muted flex flex-row items-center gap-6 transition duration-200">
